...
var current_command = {	"ability_id": "idle" }
var current_ability
...
func add_ability(a):
	abilities.append(a)
	a._on_Unit_add_ability(self)
	pass

func remove_ability(a):
	abilities.erase(a)
	a._on_Unit_remove_ability(self)
	pass

var attribute_modifiers : Array = []
func add_attribute_modifier(m):
	attribute_modifiers.append(m)
	_on_AttributeModifier_changed()
	m.connect("changed", self, "_on_AttributeModifier_changed")
	pass
func remove_attribute_modifier(m):
	attribute_modifiers.erase(m)
	_on_AttributeModifier_changed()
	m.disconnect("changed", self, "_on_AttributeModifier_changed")
	pass

func _on_AttributeModifier_changed():
	current_attributes.reset()
	current_attributes.stack_attributes(base_attributes)
	for m in attribute_modifiers:
		current_attributes.stack_attributes(m)	
	pass
	
# Called when the node enters the scene tree for the first time.
func _ready():
#	add_attribute_modifier($UnitAttributes)
	for a in $UnitAbilities.get_children():
		self.add_ability(a)
	_on_AttributeModifier_changed()
...
func set_command(c):
	...
	current_command = c
	if current_command and current_command.ability_id:
		for a in self.abilities:
			if a.ability_id == current_command.ability_id:
				current_ability = a
				current_ability.start(self, current_command)

func command_end():
	set_command({"ability_id":"idle"})
	pass
